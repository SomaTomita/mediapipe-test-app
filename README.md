...
> **これは MediaPipe のテストアプリです。** MediaPipe Tasks Vision（顔ランドマーク検出 + 手のジェスチャー認識）をブラウザ上で動かす技術検証を目的とした題材で、そのお題として対戦ミニゲームの形にしています。プロダクションを想定した製品ではありません。

遠距離カップルのための対戦ミニゲーム。ビデオ通話しながら**指ハートでハートを撃ち合い、✋ パーの手で受けとめる**。受けとめきれないと体力ゲージが減り、先に尽きた方が負け。負けた人には「愛のお題」が出ます。

![Catch My Heart のトップ画面](docs/assets/screenshot-top.png)
...
| ロビー(招待)                                | 対戦                                       | 結果                                       |
| ------------------------------------------- | ------------------------------------------ | ------------------------------------------ |
| ![ロビー画面](docs/assets/screen-lobby.png) | ![対戦画面](docs/assets/screen-battle.png) | ![結果画面](docs/assets/screen-result.png) |

> 対戦画面のカメラ映像はドキュメント用のプレースホルダーです(実際は端末のカメラ映像が表示されます)。

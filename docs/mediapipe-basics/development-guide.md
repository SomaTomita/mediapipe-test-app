...
sequenceDiagram
    ...
    M->>G: isPinched / isOpenHand / canOpenCatch
...
| --------------------- | ----------------------------------------------------- | -------------------------------------------------------- |
| 指ハート(発射)        | 幾何: 親指先(4)と人差し指先(8)の距離 ≤ 0.06           | 「ピンチ」は定型分類に存在しない                         |
| 🫴 お皿の手(キャッチ) | 幾何: `isOpenHand`(指先が手首からMCPの1.3倍以遠 ×3本) | `Open_Palm` 分類は手のひらがカメラ正対でないと外れやすい |
| 🤟 弾き返し           | 分類: `categoryName === "ILoveYou"`                   | 定型8種に含まれ、誤検出も少ない                          |
| つまみキャッチ(回復)  | 幾何: ピンチ + 顔から遠い位置                         | 同じピンチでも「顔の近さ」で発射と役割を分ける           |

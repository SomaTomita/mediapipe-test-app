...
| --------------------- | --------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| **FaceLandmarker**    | 顔の478点ランドマーク + 表情 blendshape(52種) | 鼻先(landmark 1)を取得するが、発射の「顔の近く」判定を廃止したため現状は未使用(blendshapes も無効化) |
| **GestureRecognizer** | 手の21点ランドマーク + 定型ジェスチャー分類   | 主役。指の伸び本数で 👌つまみ=回復 / 🫰指ハート=発射 を分類、🫴お皿の手判定、🤟(ILoveYou)=弾き返し   |
...
ただし定型分類はトップ1が `None` に転ぶと反応しないため、**幾何判定のフォールバック**(`src/game.ts` の `isILoveYou`: 人差し指+小指が伸び、中指+薬指が畳まれている。親指は誤検出が多いため見ない)を併用している。分類だけに頼らないのが実戦的。
...
同じく「👌 つまみ(回復)」と「🫰 指ハート(発射)」も分類ではなく幾何判定で分けている。どちらも親指×人差し指のピンチだが、**中指・薬指・小指の伸び本数**が違う(👌=3本伸び / 🫰=折り畳み)。当初は手の向き(手のひら/手の甲)で分けようとしたが、指ハートは手を斜め・エッジに向けるため向き判定が不安定だった。指の伸び具合は手の回転に強く、handedness にも依存しないため安定する(`src/game.ts` の `pinchPose`)。`?debug=1` を付けて起動すると、伸び本数・判定結果をリアルタイム表示するデバッグHUDが出る。

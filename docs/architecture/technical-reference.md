...
| **🤟 弾き返し**    | ILoveYou サインの手のひら中心(`palmCenter`)とハートの距離 ≤ 0.13                                                                               | ハートを相手に返す(紫、飛行2600ms)。**1回使うと10秒使えない**(左下バッジに残り秒数をカウントダウン表示)。**🤟中はクールダウン中でもキャッチ不可**(`canOpenCatch`。強力な技のため、回復を誤認して🤟を出すとキャッチすらできなくなるリスクを意図的にコストとして課す) |

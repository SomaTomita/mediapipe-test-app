...
| **キャッチ**       | **✋ パー(開いた手)**の手のひら(landmark 9)とハートの距離 ≤ 0.12                                                                | キャッチ成功(ダメージ回避)。**指ハートで発射準備中はキャッチ不可**                                                                                                                                                                                                  |
| **つまみキャッチ** | 顔から離れた場所でピンチしたまま、ピンチ中点との距離 ≤ 0.12(通常キャッチと同半径。つまむ動作自体が難しいため半径で不利にしない) | **体力回復 +15/個**(上限100)                                                                                                                                                                                                                                        |
| **🤟 弾き返し**    | ILoveYou サインの手のひらとハートの距離 ≤ 0.13                                                                                  | ハートを相手に返す(紫、飛行2600ms)。**1回使うと10秒使えない**(左下バッジに残り秒数をカウントダウン表示)。**🤟中はクールダウン中でもキャッチ不可**(`canOpenCatch`。強力な技のため、回復を誤認して🤟を出すとキャッチすらできなくなるリスクを意図的にコストとして課す) |
...
export const CATCH_RADIUS = 0.12;
...
export const PINCH_CATCH_RADIUS = 0.12; // 通常キャッチと同じ(つまめたら確実に取れる)
...
1. 送信側: 発射 → `heart {id, x, kind?}` 送信。自画面には発射演出のみ(物理なし)

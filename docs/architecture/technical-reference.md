...
遠距離カップルが2端末のブラウザで接続し、**🫰 手の甲を見せた指ハートでハートを撃ち合い、✋ パーで受けとめ、👌 手のひらを見せたつまみで回復する** P2P 対戦ゲーム。発射と回復は**手の向き(手のひら/手の甲)**で分ける。**体力ゲージ(HP)が先に尽きた方が負け**。ログインなし・サーバーなし・GitHub Pages ホスト。UIは「恋人のスケッチブック×エアメール」(色鉛筆の淡彩・紙の質感・手描きの揺らぎ)。
...
| 手検出   | `@mediapipe/tasks-vision` **GestureRecognizer** | **`numHands: 1`(片手プレイ前提、2本目は認識しない)**。21点 landmark + 定型ジェスチャー分類(ILoveYou=🤟)+ handedness(左右)を同時取得。handedness は手の向き(手のひら/手の甲)判定に使う |
...
- `tracker.detect(now)` の返り値(すべて非ミラー正規化座標):
  `{ nose: Point|null, hands: [{ landmarks: Point[21], iloveyou: boolean, isRight: boolean }] }`(hands は最大1本)
  - `isRight`: MediaPipe handedness が Right か。手の向き判定(`handFacing`)に使用。現実の符号が反転する場合は `handFacing` の `invert` 引数(main の `FACING_INVERT`)で校正する
...
| 操作               | 判定                                                                                                                                                                                                   | 結果                                                                                                                                                                                                                                                                |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **発射**           | 🫰 **手の甲を見せた**指ハート(ピンチ、`isFingerHeart` = facing==="back" かつ isPinched)を **指を開いて離した瞬間**                                                                                     | 通常ハート(❤️ 赤、遅め: 飛行3200ms)。連射クールダウン 800ms。**顔の近さ判定は廃止**(手の向きで発射/回復を分ける)                                                                                                                                                    |
| **チャージ弾**     | 上記の手の甲指ハートを **600ms 以上長押し**して離す                                                                                                                                                    | 💛 黄。**少しだけ速い(飛行2400ms)**、被弾ダメージ大。クールダウン 3000ms                                                                                                                                                                                            |
| **キャッチ**       | **✋ パー(開いた手)**の手のひら中心とハートの距離 ≤ 0.10(判定点は `palmCenter` = 中指MCP 9 を手首 0 へ 35% 寄せた点。付け根のままだと高すぎる)                                                         | キャッチ成功(ダメージ回避)。**指ハート(ピンチ)中・🤟中はキャッチ不可**                                                                                                                                                                                              |
| **つまみキャッチ** | 👌 **手のひらを見せた**ピンチ(`isHealPinch` = facing==="palm" かつ isPinched)のまま、ピンチ中点との距離 ≤ 0.10(通常キャッチと同半径。つまむ動作自体が難しいため半径で不利にしない)。**顔の近さは不問** | **体力回復 +15/個**(上限100)。**手のひらピンチは絶対に発射しない**                                                                                                                                                                                                  |
...
- **手の向き判定** `handFacing(landmarks, isRight, invert)`: 手首0→人差し指MCP5 と 手首0→小指MCP17 の 2D 外積の符号で手のひら/手の甲を判定(外積が `FACING_DEADZONE` 未満は unknown)。左右で符号が反転するため handedness で補正。**同じピンチでも手のひら=回復・手の甲=発射に振り分ける**のがこの判定の役割
- **向きのヒステリシス** `updateFacing`: 連続 `FACING_STABLE_FRAMES` フレーム一致で初めて向きを確定(単発ノイズで回復↔発射が入れ替わるのを防ぐ)。unknown フレームは直前状態を保持
- **発射ステートマシン** `updateShoot(state, input, now)`: 手の甲ピンチで蓄積、**指を物理的に開いた瞬間**に発射(`resolveShot` でチャージ判定)。ピンチを維持したまま手の甲→手のひらへ持ち替えたら**発射せずキャンセル**(誤爆防止)。手を見失ったフレームでは発射しない
- **クロス確証(任意)** `isThumbIndexCrossed`: 👌(ループ)と 🫰(親指×人差し指クロス)の見分けを補強する幾何判定。主判定は facing、これは発射の追加確証。main への AND 結線は手動校正で判断
...
- 画面左下: 縦バー=**チャージ長押しの進捗**(手の甲指ハート中)、🤟バッジ=**弾き返しの残りクールダウン秒数**(回復すると点滅)
- 手の骨格(21関節+ボーン)を常時描画。状態リング(`ringKind`): 🤟=紫 / 👌手のひらピンチ(回復)=赤 / 🫰手の甲ピンチ(発射)=黄 / パー=白 / その他=薄グレー
...
// 手の向き(手のひら/手の甲)判定
export const FACING_DEADZONE = 0.002; // 外積がこれ未満なら向き不定(手が端を向く)
export const FACING_STABLE_FRAMES = 4; // 連続一致で向きを確定(ヒステリシス)
export const THUMB_CROSS_MARGIN = 0.005; // 親指×人差し指クロス確証のマージン(isThumbIndexCrossed)
// ※ SPECIAL_NEAR_FACE_DIST は廃止(発射/回復の分岐を「顔の近さ」から「手の向き」へ移行)
// ※ 現実側の facing 符号は main の FACING_INVERT で校正(handFacing の invert 引数)
```
